...
public class MyPanel extends JPanel {   
	...
	public String[][] numInGrid = new String[TOTAL_COLUMNS][TOTAL_ROWS];

	public int x = -1;
	public int y = -1;
	public int mouseDownGridX = 0;
	public int mouseDownGridY = 0;
	public Color[][] field = new Color[TOTAL_COLUMNS][TOTAL_ROWS];
	public MyPanel() {   //This is the constructor... this code runs first to initialize
		...
		for (int x = 0; x < TOTAL_COLUMNS; x++) { 
			field[x][0] = Color.WHITE;
			numInGrid[x][0] =  ""; 
		}
		for (int y = 0; y < TOTAL_ROWS; y++) { 
			field[0][y] = Color.WHITE;
			numInGrid[0][y] =  ""; 
		}
		...
		for (int x = 0; x < TOTAL_COLUMNS; x++) {
			for (int y = 0; y < TOTAL_ROWS; y++) {
				if ((x == 0) || (y != TOTAL_ROWS)) {
					...
					g.drawString(numInGrid[x][y], x1 + GRID_X + (x * (INNER_CELL_SIZE + 1)) + 10, y1 + GRID_Y + (y * (INNER_CELL_SIZE + 1)) + 20);
				}
			}
		}
	}
